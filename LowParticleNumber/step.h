...
void GFstep_aGF ( struct particle *myPart, gsl_rng *r, double R, double L ){
...
void BMstep ( struct particle *particles, int *partList, double *distRow, gsl_rng *r, double tau_bm,
  ...
  for( int j =1; j<N; j ++){
    ...
    if (particles[jPart].gf==true) continue;
...
void synchPart_P_aGF ( struct particle *particles, int *partList, gsl_rng *r, int N, double Tsynch, double L ) {
  ...
  for ( int n=0; n<N; n++) {
    ...
    if (particles[n].shell>0) {
      ...
      if (Tsynch-particles[n].time> (particles[n].shell*particles[n].shell)/particles[n].Diff/100){
        ...
        double Rsynch =  drawPosNewt ( Tsynch -particles[n].time, particles[n].shell,
                                       particles[n].Diff, gsl_rng_uniform(r) );
