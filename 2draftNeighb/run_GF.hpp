void run_GF ( int N_A, int N_B, double R_A, double R_B, double D_A, double D_B, double MU_BM, double MU_GF, double tau_bm, double alpha, double Tsim, double L, double maxShell, int *stat ) {
	...
	initPos_GF ( particles, r, N_A, N_B, R_A, R_B, D_A, D_B, MU_BM, MU_GF, tau_bm, alpha, L, boxSizeGF );

    initGridGF (particles, &gridGF, NlimBox, nBoxesGF, boxSizeGF, N );

    initShell_GF ( particles, &gridGF, distRow, distLabel, r, N, tau_bm, sqrt2TAU_BM, maxShell, L, nBoxesGF, &stat[1]);
    ...
    while ( particles[partList[0]].tau_exit < Tsim ) {
		...
		updatePart_GF ( &particles[partList[0]], &gridGF, r, tau_bm, L, boxSizeGF, nBoxesGF );      
		...
		burst_P_GF ( particles, partList, distRow, distLabel, r, N, L); 

		R = getR_GF ( particles, partList, distRow, distLabel, maxShell, N, L );
