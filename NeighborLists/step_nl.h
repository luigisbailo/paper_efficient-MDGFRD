...
void GFstep_aGF_nl ( struct particle_nl *myPart, gsl_rng *r, double R, double L ){
//The shell radius "R" has been already determined with getR (),
// and it has been already checked that the domain can be constructed

  double deltaPos [3];

  //extraction of the exit time and exit position in polar coordinates
  polarTransf_nl ( deltaPos, R, gsl_rng_uniform (r), gsl_rng_uniform (r) );
  ...
  checkBound_nl (myPart->pos_exit,myPart->pos_period, L );
...
void GFstep_GF_nl ( struct particle_nl *myPart, gsl_rng *r, double R ){
//The shell radius "R" has been already determined with getR (),
// and it has been already checked that the domain can be constructed
...
void BMstep_nl ( struct particle_nl *particles, int *partList, double *distRow, int *distLabel, gsl_rng *r,
...
//sqrtTAU_BM is sqrt(2*TAU_BM)
  ...
  particles[partList[0]].pos_exit[0] = deltaPos[0] + particles[partList[0]].pos[0]
                                       + gsl_ran_gaussian (r,1)* particles[partList[0]].sqrtDiff * sqrt2TAU_BM;
  particles[partList[0]].pos_exit[1] = deltaPos[1] + particles[partList[0]].pos[1]
                                       + gsl_ran_gaussian (r,1)* particles[partList[0]].sqrtDiff * sqrt2TAU_BM;
  particles[partList[0]].pos_exit[2] = deltaPos[2] + particles[partList[0]].pos[2]
                                       + gsl_ran_gaussian (r,1)* particles[partList[0]].sqrtDiff * sqrt2TAU_BM;
  checkBound_nl (particles[partList[0]].pos_exit, particles[partList[0]].pos_period, L );
...
void synchPart_P_aGF_nl ( struct particle_nl *particles, int *partList, gsl_rng *r, int N, double Tsynch, double L ) {
    ...
    if (particles[n].shell>0) {
      ...
      if ( Tsynch < particles[n].time ){
        ...
        if (Tsynch-particles[n].time> (particles[n].shell*particles[n].shell)/particles[n].Diff/100){
          ...
          polarTransf_nl ( synchPos, Rsynch, gsl_rng_uniform (r), gsl_rng_uniform (r) );

          fixBound_aGF_nl ( particles[n].pos, particles[n].pos_exit, particles[n].pos_period, L);
          particles[n].pos[0] += synchPos[0];
          particles[n].pos[1] += synchPos[1];
          particles[n].pos[2] += synchPos[2];
          checkBound_nl (particles[n].pos,particles[n].pos_period, L );
...
void synchPart_P_GF_nl ( struct particle_nl *particles, int *partList, gsl_rng *r, int N, double Tsynch, double L ) {
