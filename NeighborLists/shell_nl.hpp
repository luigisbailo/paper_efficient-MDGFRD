double getR_aGF_nl ( particle_nl *particles, int *partList, double *distRow, int *distLabel, double maxShell, int N, double L ){
  ...
  while ( !(distLabel[j]<0) ) {
    ...
    if ( particles[jPart].gf == false ) {
      ...
      if ( particles[jPart].type == particles[iPart].type )
        ...
        Shell = distRow [j] / (1 + particles[jPart].sqrtDiff/particles[iPart].sqrtDiff );// - (particles[jPart].shellRed+particles[iPart].shellRed)/4;
      ...
      Rshell[2] = sqrt(dist2next_per_nl(&particles[iPart],&particles[jPart],L)) - particles[iPart].radius - particles[jPart].radius;
...
double getR_GF_nl ( particle_nl *particles, int *partList, double *distRow, int *distLabel, double maxShell, int N, double L ){
