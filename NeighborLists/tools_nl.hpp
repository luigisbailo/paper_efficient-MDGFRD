...
double dist2_per_nl (particle_nl *A, particle_nl *B, double L ) {
...
double dist2next_per_nl (particle_nl *A, particle_nl *B, double L ) {
...
void getDist_nl ( particle_nl *particles, std::vector<std::vector<int>> *grid, double *distRow, int *distLabel,
      ...
      for (int kk=iBoxk-1; kk<iBoxk+2; kk++){
        ...
        for(int nPart=1; nPart<(*grid)[i*nBoxes*nBoxes+j*nBoxes+k][0]+1; nPart++ ){
          ...
          if (particles[jlabel].gf==true){ 
            distRow [countDist] = sqrt(dist2_per_nl ( &particles[ilabel], &particles[jlabel], L )) - particles[ilabel].radius - particles[jlabel].radius;
          }   
            ...
            distRow [countDist] = sqrt(dist2next_per_nl ( &particles[ilabel], &particles[jlabel], L )) - particles[ilabel].radius - particles[jlabel].radius;
          }
          else {
            std::cout << "ERROR: distance in getDist" << std::endl;
          }
...
bool compareTime_nl (particle_nl A, particle_nl B)  {return A.tau_exit < B.tau_exit;}


void sortPart_nl ( particle_nl *particles, int *partList, int N) {
...
void printPos_per_nl (particle_nl *particle, int *partList, int N);
...
void updatePart_aGF_nl ( particle_nl *P, std::vector <std::vector <int>> *grid, gsl_rng *r, double dt, double L,
...
void updatePart_GF_nl ( particle_nl *P, std::vector<std::vector<int>> *grid, gsl_rng *r, double dt, double L, double boxSizeGF, int nBoxesGF ) {
...
void relabel_part_nl (particle_nl *particles, int N){
